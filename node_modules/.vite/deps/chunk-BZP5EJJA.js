...
// node_modules/.pnpm/@leafer-ui+core@1.0.10/node_modules/@leafer-ui/core/lib/core.esm.js
...
var App = class App2 extends Leafer {
  ...
  __setApp() {
    ...
    if (realCanvas || view === this.canvas.view || !canvas2.parentView)
      this.realCanvas = true;
    else
      canvas2.unrealCanvas();
    this.leafer = this;
  ...
  forceRender(bounds) {
    this.children.forEach((leafer2) => leafer2.forceRender(bounds));
  }
  addLeafer(merge) {
  ...
  __render(canvas2, options) {
    if (canvas2.context) {
      const m = options.matrix;
      if (m)
        canvas2.setTransform(m.a, m.b, m.c, m.d, m.e, m.f);
      this.children.forEach((leafer2) => canvas2.copyWorld(leafer2.canvas));
    }
  }
  __onResize(event) {
...
var DragEvent = class DragEvent2 extends PointerEvent {
  ...
  static getValidMove(leaf2, start, total) {
    ...
    const move = leaf2.getLocalPoint(total, null, true);
    move.x += start.x - x;
    move.y += start.y - y;
    if (dragBounds)
      this.getMoveInDragBounds(leaf2.__local, dragBounds === "parent" ? leaf2.parent.boxBounds : dragBounds, move, true);
    if (draggable === "x")
      move.y = 0;
    if (draggable === "y")
      move.x = 0;
...
function document(leafer2) {
  addInteractionWindow(leafer2);
  const { move, zoom } = leafer2.config;
  move.scroll = "limit";
  zoom.min = 1;
}
...
var debug$1 = Debug.get("LeaferTypeCreator");
...
var { list, register } = LeaferTypeCreator;
...
var Dragger = class {
  ...
  setDragStartPoints(list2) {
    this.dragStartPoints = {};
    list2.forEach((leaf2) => this.dragStartPoints[leaf2.innerId] = { x: leaf2.x, y: leaf2.y });
  }
  ...
  dragReal() {
    ...
    const list2 = this.realDraggableList;
    if (list2.length && running) {
      const { totalX, totalY } = this.dragData;
      list2.forEach((leaf2) => leaf2.draggable && leaf2.move(DragEvent.getValidMove(leaf2, this.dragStartPoints[leaf2.innerId], { x: totalX, y: totalY })));
    }
  }
  dragOverOrOut(data) {
    const { interaction } = this;
...
function emit(type, data, path, excludePath) {
  ...
  try {
    for (let i = path.length - 1; i > -1; i--) {
      leaf2 = path.list[i];
      if (emitEvent(leaf2, type, data, true, excludePath))
        return;
      if (leaf2.isApp)
    ...
    for (let i = 0, len = path.length; i < len; i++) {
      ...
      if (emitEvent(leaf2, type, data, false, excludePath))
        return;
    }
  } catch (e) {
    debug.error(e);
...
function emitEvent(leaf2, type, data, capture, excludePath) {
  if (leaf2.destroyed)
    return false;
...
var InteractionBase = class {
  ...
  constructor(target, canvas2, selector, userConfig) {
  ...
  multiTouch(data, list2) {
    if (this.config.multiTouch.disabled)
      return;
    const { move, angle, scale, center } = MultiTouchHelper.getData(list2);
...
var { toInnerRadiusPointOf, copy, setRadius } = PointHelper;
var inner = {};
...
leaf.__drawHitPath = function(canvas2) {
  if (canvas2)
    this.__drawRenderPath(canvas2);
};
...
rect.__hitFill = box.__hitFill = function(inner2) {
...
//# sourceMappingURL=chunk-BZP5EJJA.js.map

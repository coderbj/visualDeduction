...
// node_modules/.pnpm/@leafer-in+editor@1.0.10_@leafer-in+interface@1.0.10_@leafer-in+resize@1.0.10_@leafer-ui+core_yumj2sxned7tuz3srxhhhc3qdi/node_modules/@leafer-in/editor/dist/editor.esm.js
...
var { scalePoints } = PathScaler;
var matrix$1 = MatrixHelper.get();
var { topLeft: topLeft$1, top: top$1, topRight: topRight$1, right: right$2, bottom: bottom$1, left: left$2 } = Direction9;
function scaleResize(leaf2, scaleX, scaleY) {
  if (leaf2.pathInputed) {
    scaleResizePath(leaf2, scaleX, scaleY);
  } else {
    if (scaleX !== 1)
      leaf2.width *= scaleX;
    if (scaleY !== 1)
...
function scaleResizeFontSize(leaf2, scaleX, scaleY) {
  const { app } = leaf2;
  ...
  if (editor.editing) {
    ...
    switch (editor.resizeDirection) {
      ...
      case bottom$1:
        fontScale = scaleY;
        layout.affectScaleOrRotation ? leaf2.moveInner(-width / 2, 0) : leaf2.x -= width / 2;
        break;
      case left$2:
      case right$2:
        layout.affectScaleOrRotation ? leaf2.moveInner(0, -height / 2) : leaf2.y -= height / 2;
        break;
      case topLeft$1:
      case topRight$1:
        layout.affectScaleOrRotation ? leaf2.moveInner(0, -height) : leaf2.y -= height;
        break;
    }
  }
  leaf2.fontSize *= fontScale;
  const data = leaf2.__;
  if (!data.__autoWidth)
    leaf2.width *= fontScale;
  if (!data.__autoHeight)
    leaf2.height *= fontScale;
}
function scaleResizePath(leaf2, scaleX, scaleY) {
  PathScaler.scale(leaf2.__.path, scaleX, scaleY);
  leaf2.path = leaf2.__.path;
}
function scaleResizePoints(leaf2, scaleX, scaleY) {
  const { points } = leaf2;
  typeof points[0] === "object" ? points.forEach((p) => {
...
leaf.resizeWidth = function(width) {
  const scale2 = width / this.getBounds("box", "local").width;
  this.scaleOf(this.__layout.boxBounds, scale2, this.__.lockRatio ? scale2 : 1, true);
};
leaf.resizeHeight = function(height) {
  const scale2 = height / this.getBounds("box", "local").height;
  this.scaleOf(this.__layout.boxBounds, this.__.lockRatio ? scale2 : 1, scale2, true);
};
...
var EditorEvent = class extends Event {
  get list() {
...
var EditorMoveEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
EditorMoveEvent.MOVE = "editor.move";
var EditorScaleEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
EditorScaleEvent.SCALE = "editor.scale";
var EditorRotateEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
EditorRotateEvent.ROTATE = "editor.rotate";
var EditorSkewEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
...
var matrix = MatrixHelper.get();
var { abs } = Math;
var { copy: copy$1, scale } = MatrixHelper;
var Stroker = class extends UI {
  constructor() {
    super();
    this.list = [];
    this.hittable = false;
  ...
  __draw(canvas, options) {
    ...
    if (list2.length) {
      let leaf2;
      const data = this.__, { stroke, strokeWidth, fill } = data, { bounds: bounds2 } = options;
      for (let i = 0; i < list2.length; i++) {
        leaf2 = list2[i];
        const { worldTransform, worldRenderBounds } = leaf2;
        if (bounds2 && bounds2.hit(worldRenderBounds, options.matrix)) {
          ...
          if (aScaleX !== aScaleY) {
            ...
            const { x, y, width, height } = leaf2.__layout.boxBounds;
            canvas.rect(x * aScaleX, y * aScaleY, width * aScaleX, height * aScaleY);
          } else {
            canvas.setWorld(worldTransform, options.matrix);
            canvas.beginPath();
            if (leaf2.__.__useArrow)
              leaf2.__drawPath(canvas);
            else
              leaf2.__.__pathForRender ? leaf2.__drawRenderPath(canvas) : leaf2.__drawPathByBox(canvas);
...
var SelectArea = class extends Group {
  constructor(data) {
    super(data);
  ...
  setBounds(bounds2) {
    this.strokeArea.set(bounds2);
    this.fillArea.set(bounds2);
  }
};
var { No, Yes, NoAndSkip, YesAndSkip } = Answer;
var EditSelectHelper = {
  findOne(path) {
    return path.list.find((leaf2) => leaf2.editable);
  },
  findBounds(leaf2, bounds2) {
    if (leaf2.__.hittable && leaf2.__.visible && !leaf2.__.locked && bounds2.hit(leaf2.__world)) {
      if (leaf2.__.editable) {
        if (leaf2.isBranch && !leaf2.__.hitChildren) {
          return leaf2.__.hitSelf ? YesAndSkip : NoAndSkip;
        } else if (leaf2.isFrame) {
          return bounds2.includes(leaf2.__layout.boxBounds, leaf2.__world) ? YesAndSkip : No;
        } else {
          if (bounds2.hit(leaf2.__layout.boxBounds, leaf2.__world) && leaf2.__.hitSelf)
            return Yes;
        }
      }
      return No;
    } else {
      return leaf2.isBranch ? NoAndSkip : No;
    }
  }
};
var { findOne } = EditSelectHelper;
var EditSelect = class extends Group {
  get dragging() {
  ...
  onSelect() {
    if (this.running) {
      const { mergeConfig: config2, list: list2 } = this.editor;
      const { stroke, strokeWidth } = config2;
      this.targetStroker.setTarget(list2, { stroke, strokeWidth: Math.max(1, strokeWidth / 2) });
      this.hoverStroker.target = null;
    }
  }
  update() {
  ...
  onDrag(e) {
    ...
    if (this.dragging) {
      ...
      const list2 = new LeafList(editor.app.find(EditSelectHelper.findBounds, dragBounds));
...
var { topLeft, top, topRight, right: right$1, bottomRight, bottom, bottomLeft, left: left$1 } = Direction9;
var { toPoint } = AroundHelper;
var { within } = MathHelper;
var EditDataHelper = {
  getScaleData(element, startBounds, direction, totalMove, lockRatio, around, flipable, scaleMode) {
    let align, origin2 = {}, scaleX = 1, scaleY = 1;
      ...
      if (!unlockSide) {
        ...
        switch (direction) {
          ...
          case right$1:
            ...
            break;
          default:
            scale2 = Math.sqrt(Math.abs(scaleX * scaleY));
        }
        scaleX = scaleX < 0 ? -scale2 : scale2;
        scaleY = scaleY < 0 ? -scale2 : scale2;
      }
    }
    ...
    toPoint(around || align, boxBounds, origin2, true);
    return { origin: origin2, scaleX, scaleY, direction, lockRatio, around };
  },
  getRotateData(bounds2, direction, current, last, around) {
    let align, origin2 = {};
    switch (direction) {
      case topLeft:
        align = "bottom-right";
    ...
    toPoint(around || align, bounds2, origin2, true);
    return { origin: origin2, rotation: PointHelper.getRotation(last, origin2, current) };
  },
  getSkewData(bounds2, direction, move2, around) {
    let align, origin2 = {}, skewX = 0, skewY = 0;
    let last;
    switch (direction) {
      case top:
        last = { x: 0.5, y: 0 };
    ...
    toPoint(around || align, bounds2, origin2, true);
    const rotation = PointHelper.getRotation(last, origin2, { x: last.x + (skewX ? move2.x : 0), y: last.y + (skewY ? move2.y : 0) });
    skewX ? skewX = -rotation : skewY = rotation;
    return { origin: origin2, skewX, skewY };
  },
...
function updateMoveCursor(editor) {
  ...
  editor.editBox.rect.cursor = moveable ? moveCursor : void 0;
}
function toDataURL(svg, rotation) {
  return '"data:image/svg+xml,' + encodeURIComponent(svg.replace("{{rotation}}", rotation.toString())) + '"';
}
var EditPoint = class extends Box {
};
var fourDirection = ["top", "right", "bottom", "left"];
var EditBox = class extends Group {
  get flipped() {
  ...
  update(bounds2) {
    ...
    if (this.view.worldOpacity) {
      ...
      for (let i = 0; i < 8; i++) {
        AroundHelper.toPoint(AroundHelper.directionData[i], bounds2, point);
        resizeP = resizePoints[i];
      ...
      if (rect.path)
        rect.path = null;
      rect.set(Object.assign(Object.assign({}, bounds2), { visible: true }));
      buttons.visible = showPoints && buttons.children.length > 0;
      if (buttons.visible)
        this.layoutButtons(mergeConfig);
    }
  }
  layoutCircle(config2) {
    const { circleDirection, circleMargin, buttonsMargin, buttonsDirection, middlePoint } = config2;
  ...
  layoutButtons(config2) {
    const { buttons } = this;
    const { buttonsDirection, buttonsFixed, buttonsMargin, middlePoint } = config2;
...
function simulate(editor) {
  const { simulateTarget, list: list2 } = editor;
  const { zoomLayer } = list2[0].leafer.zoomLayer;
  simulateTarget.safeChange(() => simulateTarget.reset(bounds.setListWithFn(list2, (leaf2) => leaf2.getBounds("box", "page")).get()));
...
var order = (a, b) => a.parent.children.indexOf(a) - b.parent.children.indexOf(b);
var reverseOrder = (a, b) => b.parent.children.indexOf(b) - a.parent.children.indexOf(a);
var EditorHelper = {
  group(list2, element, userGroup) {
    list2.sort(reverseOrder);
    const { app, parent } = list2[0];
    let group;
    ...
    parent.addAt(group, parent.children.indexOf(list2[0]));
    list2.sort(order);
    ...
    app.lockLayout();
    list2.forEach((child) => child.dropTo(group));
    app.unlockLayout();
    return group;
  },
  ungroup(list2) {
    ...
    app.lockLayout();
    list2.forEach((leaf2) => {
      if (leaf2.isBranch && !leaf2.isBranchLeaf) {
        const { parent, children } = leaf2;
        while (children.length) {
          ungroupList.push(children[0]);
          children[0].dropTo(parent, parent.children.indexOf(leaf2));
        }
        leaf2.remove();
      } else {
        ungroupList.push(leaf2);
      }
    });
    app.unlockLayout();
    return ungroupList;
  ...
  toBottom(list2) {
    ...
    list2.forEach((leaf2) => {
      if (leaf2.parent)
        leaf2.parent.addAt(leaf2, 0);
    });
  }
};
var debug = Debug.get("EditToolCreator");
function registerEditTool() {
  return (target) => {
    EditToolCreator.register(target);
  };
}
var registerInnerEditor = registerEditTool;
var EditToolCreator = {
  list: {},
  register(EditTool3) {
    const { tag } = EditTool3.prototype;
...
var { list } = EditToolCreator;
var InnerEditorEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
...
var EditorGroupEvent = class extends EditorEvent {
  constructor(type, data) {
    super(type, data);
  }
};
...
var { updateMatrix } = LeafHelper;
var checkMap = { x: 1, y: 1, scaleX: 1, scaleY: 1, rotation: 1, skewX: 1, skewY: 1 };
var origin = "top-left";
var SimulateElement = class extends Rect {
  get __tag() {
...
var Editor = class extends Group {
  get mergeConfig() {
    const { element, config: config2 } = this;
    return this.single && element.editConfig ? Object.assign(Object.assign({}, config2), element.editConfig) : config2;
  }
  get list() {
  ...
  onMove(e) {
    if (e instanceof MoveEvent) {
      if (e.moveType !== "drag") {
        ...
        const move2 = e.getLocalMove(this.element);
        if (moveable === "move")
          e.stop(), this.move(move2.x, move2.y);
  ...
  onRotate(e) {
    ...
    const { element } = this;
    let origin2, rotation;
    if (e instanceof RotateEvent) {
      if (rotateable === "rotate")
        e.stop(), rotation = e.rotation, origin2 = element.getBoxPoint(e);
      else
        return;
    } else {
    ...
    if (element.scaleX * element.scaleY < 0)
      rotation = -rotation;
    this.rotateOf(origin2, MathHelper.float(rotation, 2));
  }
  onSkew(e) {
    const { element } = this;
    const { around } = this.mergeConfig;
    const { origin: origin2, skewX, skewY } = EditDataHelper.getSkewData(element.boxBounds, e.current.direction, e.getInnerMove(element), EditDataHelper.getAround(around, e.altKey));
    if (!skewX && !skewY)
      return;
    this.skewOf(origin2, skewX, skewY);
  }
  move(x, y = 0) {
  ...
  scaleOf(origin2, scaleX, scaleY = scaleX, _resize) {
    if (!this.checkTransform("resizeable"))
      return;
    const { element } = this;
    const worldOrigin = this.getWorldOrigin(origin2);
    const transform = this.multiple && this.getChangedTransform(() => element.safeChange(() => element.scaleOf(origin2, scaleX, scaleY)));
  ...
  rotateOf(origin2, rotation) {
    ...
    const { element } = this;
    const worldOrigin = this.getWorldOrigin(origin2);
    const transform = this.multiple && this.getChangedTransform(() => element.safeChange(() => element.rotateOf(origin2, rotation)));
    const event = new EditorRotateEvent(EditorRotateEvent.ROTATE, { target: element, editor: this, worldOrigin, rotation, transform });
    this.editTool.onRotate(event);
    this.emitEvent(event);
  }
  skewOf(origin2, skewX, skewY = 0, _resize) {
    if (!this.checkTransform("skewable"))
      return;
    const { element } = this;
    const worldOrigin = this.getWorldOrigin(origin2);
    const transform = this.multiple && this.getChangedTransform(() => element.safeChange(() => element.skewOf(origin2, skewX, skewY)));
    const event = new EditorSkewEvent(EditorSkewEvent.SKEW, { target: element, editor: this, worldOrigin, skewX, skewY, transform });
    this.editTool.onSkew(event);
    this.emitEvent(event);
  }
  checkTransform(type) {
    return this.element && !this.element.locked && this.mergeConfig[type];
  }
  getWorldOrigin(origin2) {
    return this.element.getWorldPoint(LeafHelper.getInnerOrigin(this.element, origin2));
  }
  ...
  ungroup() {
    const { list: list2 } = this;
    if (list2.length) {
      list2.forEach((item) => item.isBranch && this.emitGroupEvent(EditorGroupEvent.BEFORE_UNGROUP, item));
      this.target = EditorHelper.ungroup(list2);
      list2.forEach((item) => item.isBranch && this.emitGroupEvent(EditorGroupEvent.UNGROUP, item));
    }
    return this.list;
  }
  openGroup(group) {
  ...
  checkOpenedGroups() {
    ...
    if (opened.length) {
      let { list: list2 } = opened;
      if (this.editing)
        list2 = [], opened.forEach((item) => this.list.every((leaf2) => !LeafHelper.hasParent(leaf2, item)) && list2.push(item));
      list2.forEach((item) => this.closeGroup(item));
    }
    if (this.editing && !this.selector.dragging)
      this.checkDeepSelect();
  }
  checkDeepSelect() {
    let parent, { list: list2 } = this;
    for (let i = 0; i < list2.length; i++) {
      parent = list2[i].parent;
  ...
  lock() {
    this.list.forEach((leaf2) => leaf2.locked = true);
    this.update();
  }
  unlock() {
    this.list.forEach((leaf2) => leaf2.locked = false);
    this.update();
  }
  toTop() {
    if (this.list.length) {
  ...
  listenTargetEvents() {
    if (!this.targetEventIds.length) {
      ...
      this.targetEventIds = [
        ...
        app.on_([KeyEvent.HOLD, KeyEvent.UP], (e) => {
          updateCursor(this, e);
        }),
...
var EditTool = class EditTool2 extends InnerEditor {
...
var { left, right } = Direction9;
var { move, copy, toNumberPoints } = PointHelper;
var LineEditTool = class LineEditTool2 extends EditTool {
  constructor() {
    super(...arguments);
  ...
  onScaleWithDrag(e) {
    ...
    if (line.pathInputed) {
      ...
      const { toPoint: toPoint2 } = line;
      line.rotation = 0;
      this.dragPoint(from, toPoint2, isDragFrom, around, this.getInnerMove(line, drag, lockRatio));
      line.getLocalPointByInner(from, null, null, true);
      line.getLocalPointByInner(toPoint2, null, null, true);
      line.x = from.x;
      line.y = from.y;
      line.getInnerPointByLocal(toPoint2, null, null, true);
      line.toPoint = toPoint2;
    }
  }
  ...
  dragPoint(fromPoint, toPoint2, isDragFrom, around, movePoint) {
    const { x, y } = movePoint;
    if (isDragFrom) {
      move(fromPoint, x, y);
      if (around)
...
UI.setEditConfig = function(config2) {
...
//# sourceMappingURL=chunk-WCIMPE26.js.map

...
// node_modules/.pnpm/@leafer-in+state@1.0.10_@leafer-in+interface@1.0.10_@leafer-ui+core@1.0.10_@leafer-ui+interface@1.0.10/node_modules/@leafer-in/state/dist/state.esm.js
...
function updateStyle(leaf, style, type) {
  ...
  const fromStyle = transition ? getFromStyle(leaf, style) : void 0;
  ...
  if (statesStyle) {
    ...
    if (animation) {
      const animate = leaf.animate(animation, void 0, "animation", true);
...
function setChildrenState(children, stateType2, state) {
  if (!children)
    return;
  let leaf, update;
  for (let i = 0, len = children.length; i < len; i++) {
    ...
    if (stateType2) {
      ...
      switch (stateType2) {
        case "hoverStyle":
          if (State.isHover(leaf))
            update = false;
          break;
        case "pressStyle":
          if (State.isPress(leaf))
            update = false;
          break;
        case "focusStyle":
          if (State.isFocus(leaf))
            update = false;
      }
      if (update)
        setPointerState(leaf, stateType2);
    } else if (state)
      setState(leaf, state);
    if (leaf.isBranch)
      setChildrenState(leaf.children, stateType2, state);
  }
}
...
function unsetChildrenState(children, stateType2, state) {
  if (!children)
    return;
  let leaf;
  for (let i = 0, len = children.length; i < len; i++) {
    leaf = children[i];
    if (stateType2)
      unsetPointerState(leaf, stateType2);
    else if (state)
      unsetState(leaf, state);
    if (leaf.isBranch)
      unsetChildrenState(leaf.children, stateType2, state);
  }
}
...
State.setStyleName = function(leaf, stateType2, value) {
  value ? setState(leaf, stateType2, leaf[stateType2]) : unsetState(leaf, stateType2, leaf[stateType2]);
};
...
ui.updateState = function() {
  State.updateStyle(this, void 0, "in");
};
export {
  stateStyleType,
  stateType
};
//# sourceMappingURL=@leafer-in_state.js.map
